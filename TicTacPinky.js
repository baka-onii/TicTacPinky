...
function handleMiniCellClick(largeIndex, miniIndex) {
    if (BBoard[largeIndex].cells[miniIndex] === null && BBoard[largeIndex].winner === null && turn === playerMark) {
        ...
        const largeBoardWinner = checkGameWinner();
        if (largeBoardWinner) {
            // Handle game end
            turnMessage.textContent = `${largeBoardWinner === playerMark ? playerName : opponentName} wins the game!`;
            endGame();
            return;
        }
